...
import re
import time
from tutorial.items import Entitem
...
class Entspider(scrapy.Spider):
    ...
    start_urls = [
        "http://openapi.inews.qq.com/getQQNewsIndexAndItems?chlid=news_news_ent&refer=mobilewwwqqcom&otype=jsonp&t=1459166351668"
    ]

    def parse(self, response):

        pattern = re.compile(r"http://xw\.qq\.com/ent/.+")
        if re.match(pattern, response.url):
            ...
            for news in sites['idlist'][0]['ids']:
                ...
                url = "http://xw.qq.com/ent/" + news['id'][3:17] + "/" + news['id']
